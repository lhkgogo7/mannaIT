...
public class EquipmentDAO {
	...
	public int getEquipmentTotal(int eq_ca_code) {
		String sql = "SELECT COUNT(*) FROM EQUIPMENT EQ "
				...
				+ " ON EC.EQ_CA_CODE = EQ.EQ_EQCACODE WHERE EQ_CA_CODE="
				+ eq_ca_code;
		try {
	...
	public Vector<EquipmentBean> getEquipmentList(int ca_code, int cur_page,
		...
		try {
			...
			/*
			 * sql =
			 * "SELECT * FROM (SELECT ROWNUM RNUM, EQ_CODE, EQ_NAME, EQ_MANUFACTURER, EC.EQ_CA_NAME,EQ_CA_CODE, TO_CHAR(EQ_DATE, 'yyyy-mm-dd')"
			 * + " FROM EQUIPMENT EQ" + " INNER JOIN EQ_CATEGORY EC" +
			...
			sql = "SELECT * FROM (SELECT ROWNUM RNUM,EQ_CODE, EQ_NAME, EQ_MANUFACTURER, EQ_CA_NAME,EQ_CA_CODE,E_DATE , EQ_PICTURE, EQ_USER "
					...
					+ " WHERE RNUM >="
					+ start_row
					+ " AND RNUM<=" + end_row + " ORDER BY RNUM ASC";
			msg = sql;
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			// 해당 값을 얻는다.
	...
	public Vector<EquipmentBean> getEquipmentList(int cur_page, int limit) {
		...
		try {
			...
			int end_row = cur_page * limit;
			System.out
					.println("getEquipmentList(int cur_page,int limit) endrow:"
							+ end_row + "start_row:" + start_row);

			/*
			 * sql =
			 * "SELECT * FROM (SELECT ROWNUM RNUM, EQ_CODE, EQ_NAME, EQ_MANUFACTURER, EC.EQ_CA_NAME, EQ_CA_CODE, TO_CHAR(EQ_DATE, 'YYYY-MM-DD'), EQ_PICTURE"
			 * + " FROM EQUIPMENT EQ" + " INNER JOIN EQ_CATEGORY EC" +
	...
	public boolean modifyEquipment(EquipmentBean eb) {
		...
		try {
			...
			sql = " UPDATE EQUIPMENT" + " set EQ_NAME = ? ,"
					...
					+ " EQ_DATE = ?," + " EQ_PICTURE =?" + " WHERE EQ_CODE=?";
			...
			pstmt.setString(2, eb.getManufacturer());
			System.out.println("  eb.getManufacturer()::"
					+ eb.getManufacturer());
			pstmt.setInt(3, eb.getEq_ca_code());
	...
	public Vector<EquipmentBean> getRentalList(int ca_code) {
		...
		try {
			...
			sql = "SELECT RENT_CODE, EQ_NAME, M_NAME,  TO_CHAR(RENT_SDATE, 'yyyy-mm-dd'), TO_CHAR(RENT_EDATE, 'yyyy-mm-dd')"
					...
					+ " ON R.RENT_EQCODE=E.EQ_CODE"
					+ " INNER JOIN MEMBER M "
					+ " ON M.M_CODE = R.RENT_MCODE" + " WHERE E.EQ_EQCACODE=? ";
			msg = sql;
			pstmt = con.prepareStatement(sql);
	...
	public boolean updateRentalDate(int rt_code, String e_date ) {
		try {
			iscon = con.isClosed();
			if (iscon) {
				connect();
		...
		try {
			System.out.println("updateRentalDate 진입");

			sql = "UPDATE RENTAL SET RENT_EDATE=TO_DATE(?, 'YYYY-MM-DD') WHERE RENT_CODE=?";
			...
			pstmt.setInt(2, rt_code);
			
			res = pstmt.executeUpdate();

			System.out.println("updateRentalDate res : " + res);
			if (res == 0) {
				return false;
			}
